...
const CartItems = () => {
  const products = useContext(ShopContext)?.all_product;
  const cartItems = useContext(ShopContext)?.cartItems;
  const removeFromCart = useContext(ShopContext)?.removeFromCart;
  const getTotalCartAmount = useContext(ShopContext)?.getTotalCartAmount;
  if (products === undefined) return null;
  if (cartItems === undefined) return null;
  if (removeFromCart === undefined) return null;
  if (getTotalCartAmount === undefined) return null;
